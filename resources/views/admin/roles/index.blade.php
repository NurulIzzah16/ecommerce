...
@section('content')
    ...
    <div class="button-action" style="margin-bottom: 20px">
        @if (!in_array('roles.create', auth()->user()->role->permissions ?? []))
        ...
        <tbody>
            @foreach ($roles as $role)
                <tr>
                    ...
                    <td>{{ $role->name }}</td>
                    <td>
                        @if (!in_array('roles.edit', auth()->user()->role->permissions ?? []))
                        <a href="{{ route('roles.edit', $role->id) }}" class="btn btn-sm btn-primary">Edit</a>
                        @endif
                        @if (!in_array('roles.delete', auth()->user()->role->permissions ?? []))
